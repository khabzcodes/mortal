...
import { createNoteValidation, updateContentValidation } from "@/validations/notes";
...
export const notes = new Hono<{
  ...
  .get("/", async (c) => {
    try {
      ...
      const notes = await notesRepository.selectNotesByOrganizationId(organizationId);
  ...
  .put("/:id/content", zValidator("json", updateContentValidation), async (c) => {
    try {
      const session = c.get("session");
      const member = c.get("member");
      if (!session || !session.session.activeOrganizationId || !member) {
      ...
      const note = await notesRepository.updateNoteContent(noteId, content, member.id);

      if (!note) {
        return c.json({ message: "Note not found" }, 404);
      }

      const isContributor = noteExists.contributors.some((contributor) => contributor.user.id === session.user.id);
